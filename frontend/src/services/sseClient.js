/**
 * 📡 Centralized Chunk-Safe SSE Client (Issue #18 & Issue #19)
 * 
 ...
 * - Ignores SSE comment lines (e.g. ": keep-alive") without corrupting state
 ...
 * and yields parsed event objects.
 * 
  ...
  try {
    while (true) {
      ...
      for (const rawLine of lines) {
        ...
        // Ignore comments (like ": keep-alive")
        if (line.startsWith(':')) {
        ...
        if (line.startsWith('data: ') || line === 'data:') {
          ...
          try {
            ...
            console.warn('[SSE] Incomplete or non-JSON data line encountered:', dataStr, jsonErr);
          }
        }
      }
    }
      ...
      if (line.startsWith('data: ') || line === 'data:') {
        ...
        if (dataStr && dataStr !== '[DONE]') {
          try {
            ...
            console.warn('[SSE] Trailing non-JSON data line:', dataStr, jsonErr);
          }
        }
      }
    }
