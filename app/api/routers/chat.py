...
async def get_backend_status():
    """
    Zeigt Status aller verfügbaren Backends (Hailo, vLLM, Ollama, llama.cpp).
    ...
    Returns:
        ...
        - Recommendation für aktuellen Zustand
    """
    try:
        backend_router = get_backend_router()
        
        # Get status of all backends (already returns formatted dict)
        backends = await backend_router.get_backend_status()
        
        # Count healthy backends
        healthy_backends = [name for name, info in backends.items() if info.get("healthy", False)]
        
        # Generate recommendation
        if not healthy_backends:
            recommendation = "⚠️ Keine Backends verfügbar! Bitte Systemstatus prüfen."
        elif "hailo" in healthy_backends:
            recommendation = "✅ Hailo verfügbar - optimale Performance für Vision-Tasks (5ms, 6W)"
        elif "vllm" in healthy_backends:
            recommendation = "✅ vLLM verfügbar - beste Performance für Chat (50ms, GPU)"
        elif "ollama" in healthy_backends:
            recommendation = "✅ Ollama verfügbar - solide lokale Inferenz (100ms, 20W)"
        else:
            recommendation = "⚠️ Nur llama.cpp verfügbar - langsame CPU Inferenz (500ms)"
        
        from datetime import datetime
        
        return {
            "backends": backends,
            "healthy_count": len(healthy_backends),
        ...
        return {
            ...
            "healthy_count": 0,
            "total_count": 0,
            "recommendation": "❌ Backend Router nicht verfügbar"
        }
...
async def quick_ask(
    ...
    """Schnelle Frage an Liara."""
    try:
        result = await chat_with_liara(ChatRequest(message=message), current_user)
        return {"answer": result.response}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


# Globale Variable für ausgewähltes Modell
...
async def select_model(
    ...
    """Setze Standard-Modell für Chat."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if request.model not in available_names:
                raise HTTPException(
                    status_code=404,
                    detail=f"Model '{request.model}' not found. Available: {available_names}"
                )
            ...
            return {
                ...
                "message": f"Default model set to {SELECTED_MODEL}"
            }
...
async def get_current_model(current_user: User = Depends(require_active_user)):
    """Zeige aktuell ausgewähltes Modell."""
    return {
        "selected_model": SELECTED_MODEL
    }
...
async def get_models_summary(current_user: User = Depends(require_active_user)):
    """Vereinfachte Model-Liste für UI."""
    try:
        response = requests.get("http://localhost:11434/api/tags", timeout=2)
        if response.status_code != 200:
        ...
        for model in models:
            ...
            summary.append({
                "name": name,
                "size_gb": size_gb,
                "recommended": size_gb < safe_limit,
                "selected": name == SELECTED_MODEL
            })
        
        # Sortiere: Empfohlen zuerst, dann nach Größe
        summary.sort(key=lambda x: (not x["recommended"], x["size_gb"]))
